(function(){
  ...
  function normalizePaperCode(paperVal) {
    ...
    if (str.includes('GS2') || str.includes('PAPER2') || str.includes('POLITY') || str.includes('GOVERNANCE')) return 'GS2';
  ...
  function populateFilterYears() {
    ...
    const yearsInBank = [...new Set(presetBank.map(q => String(q.year)).filter(y => y && y !== 'undefined'))];
  ...
  if(clearBankBtn) {
    clearBankBtn.addEventListener('click', async () => {
      if (confirm('Reset and reload default questions from files?')) {
        ...
        updateBankStatus();
        if(bankResultsContainer) bankResultsContainer.innerHTML = '<p style="font-size:13px; color:var(--ink-soft); font-style:italic;">Reloading questions...</p>';
  ...
  if(addBtn) {
    addBtn.addEventListener('click', () => {
      ...
      const item = { question: text, marks: selectedMarks, year: qYear ? qYear.value.trim() : '', paper: qPaper ? qPaper.value : 'GS1', topic: qTopic ? qTopic.value : 'General' };
  ...
  if(addPresetBtn) {
    addPresetBtn.addEventListener('click', () => {
      const text = qText ? qText.value.trim() : '';
      if (!text) { if(qText) qText.focus(); return; }
      const newPreset = { id: 'p_' + Date.now(), paper: qPaper ? qPaper.value : 'GS1', topic: qTopic ? qTopic.value : 'General', year: qYear ? qYear.value.trim() || '2025' : '2025', marks: selectedMarks, question: text };
  ...
  function generatePDF() {
    ...
    if (includeIndexEl && includeIndexEl.checked) {
      ...
      doc.text('Q#', 28, y); doc.text('Question', 40, y); doc.text('Marks', 170, y);
      y += 4; doc.line(28, y, 180, y); y += 6;
